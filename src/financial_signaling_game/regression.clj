...
(def reg-investamt-ds (-> real-data
                          (dissoc :Num_Investors :lNum_Investors :Success :Compan_Status)
...
(def reg-num-investor-ds (-> real-data
                             (dissoc :Num_Investors :lSuccess :Success :Company_Status)
                             (ds/categorical->number cf/categorical)))
...
(def mse-num (loss/mse (predict-num :lNum_Investors) 
...
(defn stat-amt
  "print out the regression outcome of investment amount"
    [] 
  (let [reg-data (ds/value-reader real-data)
        amt-investment (sel reg-data :cols 12)
        x (sel reg-data :cols '(0 1 2 3 6 7 8 9 10))]
...
(defn stat-num
  ...
  []
  (let [reg-data (ds/value-reader real-data)
        num-investor (sel reg-data :cols 13)
        x (sel reg-data :cols '(0 1 2 3 6 7 8 9 10))]
