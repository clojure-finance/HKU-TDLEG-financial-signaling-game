...
(defn visual-control
  [ds]  
  (with-data (->> (-> ds (importdata) (choosedate) (indepvar) (depvar) (ds/mapseq-reader) (seq) (to-dataset))
                 ($rollup :count :Share_Ratio [:Location :Industry_Sector :Company_Status]))
   (save (stacked-bar-chart :Location :Share_Ratio :title "Location Distribution"  :group-by :Location :legend true) "resources/visual-location.png")
