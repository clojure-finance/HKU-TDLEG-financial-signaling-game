...
(:use (incanter core stats datasets charts io svg)))
...
"Produce pie charts depicting investee companies' status using incanter"  
  ...
  (let [control-pivot (->> (-> ds (importdata) (choosedate) (indepvar) (controlvar) (depvar)
    ...
    (with-data (->> control-pivot
       ...
       (save-svg (pie-chart
              :years :Share_Ratio
              :title "Year Distribution") "resources/visual-years.svg"))
    (with-data (->> control-pivot
                    ($rollup :sum :Share_Ratio :Location))
      (save-svg (pie-chart
             :Location :Share_Ratio
             :title "Location Distribution") "resources/visual-location.svg"))
    (with-data (->> control-pivot
                    ($rollup :sum :Share_Ratio :Company_Status))
      (save-svg (pie-chart :Company_Status :Share_Ratio
                       :title "Company Status Distribution") "resources/visual-company-status.svg"))
    (with-data (->> control-pivot
                ($rollup :sum :Share_Ratio :Industry_Sector))
      (save-svg (pie-chart :Industry_Sector :Share_Ratio
                   :title "Industry Distribution") "resources/visual-industry.svg"))))
