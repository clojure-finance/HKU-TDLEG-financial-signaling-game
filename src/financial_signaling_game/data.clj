...
(defn importregdata
  ...
  (-> ds
      (dissoc
       ...
       :Disclosed_Equity_Contribution)
      (assoc :Investor_Equity_Total (dfn/log (ds :Investor_Equity_Total)))
...
(defn visual-amt
"Visualizing relationship between share ratio and investor amount"  
  [ds]
...
(defn visualize
  ...
  []
  (let [ds raw-data]
   (description (processing ds))
    (visual-amt ds)
    (visual-num ds)
    (visual-control ds)
    (ds/write! (pivot-table ds) "resources/pivot-table.csv")
    (visual-pivot-table ds)
    (visual-relationship (processing ds))
    (ds/write! (processing ds) "resources/regression-data.csv")))